...
function showModalImage() {
  ...
  if (modalImages.length <= 1) {
    ...
    prev.style.display = modalIndex > 0 ? '' : 'none';
    next.style.display = modalIndex < modalImages.length - 1 ? '' : 'none';
  }
}
...
function initLocation() {
  ...
  mapImg.src = 'images/location/1.png';

  // 지도 이미지 클릭 시 확대
  mapImg.style.cursor = 'zoom-in';
  mapImg.addEventListener('click', () => {
    openPhotoModal(['images/location/1.png'], 0);
  });
